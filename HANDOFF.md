...
- 2026-06-25 用户指出两位老师 v1 “动作不连贯、太丑”。已新增可复现脚本 `tools/generate-teacher-sprites.mjs`、`tools/make-godot-ctex-webp.mjs`、`tools/replace-godot-pck-asset.mjs`，用固定 48×48 单元格和底部锚点生成 v2 Q 版 atlas，再替换 Godot PCK 中 `mario_sheet.png-...ctex`。线上 KV 已覆盖原 key：`liushuo` PCK 9,400,931 bytes，`guoliang` PCK 9,397,123 bytes；预览 PNG 分别 130,454 / 126,784 bytes。Worker 的素材 size normalize 上限从 8MB 调到 20MB，避免 manifest 把 PCK size 截断成 8,388,608。
